...
Troy adds elliptical ripples and soft blue orbital contours to the shared sea,
and Delphi has concentric basin ripples. Each scene returns to its initial
state at 10 seconds, so the encoded `[0,10)` frames form a continuous loop.
