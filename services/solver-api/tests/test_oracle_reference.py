...
from app.oracle.models import Axes, PriceReference, ReferenceRequest, content_id
...
class OracleReferenceTests(unittest.TestCase):
    ...
    def test_cancel_between_payoff_blocks(self):
        ...
        try:
            with patch("app.oracle.reference._discounted_payoffs", side_effect=payoff):
    ...
    def test_expired_deadline_prevents_allocation(self):
        ...
        try:
            with patch("app.oracle.reference._normal_samples") as draw:
                with self.assertRaises(ExecutionStopped):
