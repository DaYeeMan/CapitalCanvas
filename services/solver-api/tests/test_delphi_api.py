...
from app.delphi.models import default_reference_request
from app.delphi.router import MAX_BODY_BYTES, read_model
from app.delphi.models import ReferenceRequest
from app.runner import run_bounded
from delphi_fixtures import small_reference


class DelphiAPITests(unittest.TestCase):
    def setUp(self):
        self.client = TestClient(app)

    def test_reference_and_all_model_flow(self):
        reference = self.client.post("/v1/delphi/reference", json=small_reference().configuration.model_dump(mode="json"))
        self.assertEqual(reference.status_code, 200, reference.text)
        result = self.client.post("/v1/delphi/experiment", json={"reference": reference.json(), "settings": {"mlp": {"epochs": 20}}})
    ...
    def test_iv_and_extrapolation_api_flow(self):
        ...
        reference = self.client.post("/v1/delphi/reference", json=request).json()
        result = self.client.post("/v1/delphi/experiment", json={"reference": reference, "target": "implied_volatility", "sampling": {"budget": 32, "training_bounds": {"spot_min": 85., "spot_max": 115., "tau_min": .4, "tau_max": 1.8}}, "settings": {"mlp": {"epochs": 20}}})
        self.assertEqual(result.status_code, 200, result.text)
    ...
    def test_capabilities_and_bad_requests(self):
        capabilities = self.client.get("/v1/delphi/capabilities").json()
        self.assertEqual(len(capabilities["methods"]), 4)
        self.assertEqual(capabilities["limits"]["body_bytes"], MAX_BODY_BYTES)
        response = self.client.post("/v1/delphi/reference", json={"monte_carlo": {"paths": 100000}})
        self.assertEqual(response.status_code, 422)
        response = self.client.post("/v1/delphi/reference", content=b"{" + b" " * MAX_BODY_BYTES)
        self.assertEqual(response.status_code, 413)
        reference = small_reference().model_dump(mode="json")
        reference["prices"][1][0] += .1
        response = self.client.post("/v1/delphi/experiment", json={"reference": reference})
        self.assertEqual(response.status_code, 422)
        self.assertIn("identity mismatch", response.text)

    def test_invalid_iv_training_fails_shared_experiment(self):
        result = self.client.post("/v1/delphi/experiment", json={"reference": small_reference().model_dump(mode="json"), "target": "implied_volatility"})
        self.assertEqual(result.status_code, 422)
        self.assertEqual(result.json()["error"]["code"], "invalid_training_nodes")

    def test_timeout_uses_structured_error(self):
        with patch("app.delphi.router.generate_reference", side_effect=ExecutionStopped("deadline")):
            response = self.client.post("/v1/delphi/reference", json={})
