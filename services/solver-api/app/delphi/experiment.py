...
from app.delphi import mlp
from app.delphi.baseline import BASELINE_VERSION, evaluate_baseline, reconstruct, residual_targets
from app.delphi.contracts import Diagnostics, ExperimentRequest, ExperimentResult, Snapshot, SurrogateResult
from app.delphi.evaluation import evaluate_predictions
from app.delphi.implied_volatility import build_target
from app.delphi.models import NumericalError, content_id
from app.delphi.preprocessing import training_transform
from app.delphi.sampling import sample_training
from app.delphi.surrogates import fit_gp, fit_spline

logger = logging.getLogger("ithaca.delphi")
...
def run_experiment(request: ExperimentRequest) -> ExperimentResult:
    ...
    for method in request.methods:
        ...
        try:
            ...
            logger.warning("Delphi method %s failed: %s", method, error)
    ...
    identity = {"version": "delphi-fit-v1", "reference_id": reference.reference_id, "sample_set_id": samples.sample_set_id, "configuration": snapshot.model_dump(mode="json")}
