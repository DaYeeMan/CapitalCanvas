...
it("supports arrow keys and skips disabled uncertainty", () => {
  const change = vi.fn();
  render(<OracleTabs label="Views" panelId="plot" value="reference" onChange={change} options={[{ value: "reference", label: "Reference" }, { value: "uncertainty", label: "GP uncertainty", disabled: true, reason: "Fit GP first" }, { value: "error", label: "Error" }]} />);
...
it("shows failed methods without invented metrics and converts decimal IV errors into volatility points", () => {
  ...
  render(<OracleComparison result={result} active="cubic_spline" onSelect={vi.fn()} region="full" onRegion={vi.fn()} />);
