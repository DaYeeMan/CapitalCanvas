...
import { OracleMethodDetails } from "./OracleMethodDetails";
...
export function OracleSurface({ result, method, view, slice, onSlice }: { result: ExperimentResult; method: Method; view: View; slice: boolean; onSlice: (slice: boolean) => void }) {
  ...
  return <>
    <section className="oracle-surface-panel" aria-label={title}>
      <div className="oracle-visualization">
        <div className="oracle-plot" id="oracle-surface-plot" role="tabpanel" aria-label={slice ? "Surface slice" : title}>
          {slice ? <ScientificPlot data={sliceData} layout={sliceLayout} label={`Price or IV slice at ${tau} years; reference and selected surrogate predictions`} /> : field ? <ScientificPlot preserveCamera data={data} layout={layout} label={`${title}, spot versus maturity. ${result.samples.count} training samples; ${invalid} masked reference nodes.`} /> : <div className="oracle-empty">This model has no completed surface. Select a completed method or rerun the experiment.</div>}
        </div>
        <div className="oracle-plot-controls"><label className="oracle-check"><input type="checkbox" checked={overlay} disabled={slice} onChange={event => setOverlay(event.target.checked)} />Training points</label><button aria-pressed={slice} onClick={() => onSlice(!slice)}>{slice ? "3D surface" : "2D slice"}</button></div>
        {slice ? <div className="oracle-slice-controls"><label>Slice maturity: {numberText(tau, 3)} yr<input aria-label="Slice maturity" type="range" min={0} max={result.axes.times_to_maturity.length - 1} value={row} onChange={event => setSliceIndex(Number(event.target.value))} /></label>{result.methods.some(item => item.predictive_stddev) ? <label className="oracle-check"><input type="checkbox" checked={band} onChange={event => setBand(event.target.checked)} />GP mean ± 1.96 SD (model uncertainty)</label> : null}<details className="oracle-disclosure"><summary>Slice values · accessible table</summary><div className="oracle-table-scroll" tabIndex={0}><table><thead><tr><th>Spot</th><th>MC reference</th>{result.methods.map(model => <th key={model.method}>{labels[model.method]}</th>)}</tr></thead><tbody>{result.axes.spots.map((spot, column) => <tr key={spot}><td>{numberText(spot, 2)}</td><td>{numberText(result.target.values[row][column] == null ? null : result.target.values[row][column]! * factor)}</td>{result.methods.map(model => <td key={model.method}>{numberText(model.predictions?.[row][column] == null ? null : model.predictions[row][column]! * factor)}</td>)}</tr>)}</tbody></table></div></details></div> : null}
      </div><OracleMethodDetails result={result} method={method} />
    </section>
    {result.configuration_snapshot.sampling.training_bounds ? <div className="oracle-domain-summary"><svg viewBox="0 0 260 100" role="img" aria-label="Training rectangle inside reference domain; shaded rectangle is interpolation region"><rect x="20" y="8" width="220" height="66" fill="#061725" stroke="#334452" /><rect x={20 + (b.spot_min - domain.spot_min) / (domain.spot_max - domain.spot_min) * 220} y={8 + (domain.tau_max - b.tau_max) / (domain.tau_max - domain.tau_min) * 66} width={(b.spot_max - b.spot_min) / (domain.spot_max - domain.spot_min) * 220} height={(b.tau_max - b.tau_min) / (domain.tau_max - domain.tau_min) * 66} fill="#123e4d" stroke="#24d5e7" /><text x="130" y="92" textAnchor="middle" fill="#a8b4bd" fontSize="11">Spot · maturity domain</text></svg><p>Realized training region: S {numberText(b.spot_min, 2)}–{numberText(b.spot_max, 2)}, τ {numberText(b.tau_min, 3)}–{numberText(b.tau_max, 3)} yr.<br /><span className="oracle-caption">Shaded region interpolates. Remaining valid nodes extrapolate. {slice && (tau < b.tau_min || tau > b.tau_max) ? "This entire maturity slice lies outside the training region." : ""}</span></p></div> : null}
    <p className="oracle-caption">{invalid} masked reference nodes · Maximum {isIV ? "approximate IV" : "MC price"} standard error: {numberText(maxSE)} {isIV ? "vol pts" : "$"}. {view === "error" ? "Error color scale is shared across completed models." : "Monte Carlo is a high-fidelity reference, not exact truth."}</p>
  </>;
}
