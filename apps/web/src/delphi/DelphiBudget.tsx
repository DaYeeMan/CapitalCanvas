...
export function DelphiBudget({ points, onSelect }: { points: BudgetPoint[]; onSelect: (result: ExperimentResult) => void }) {
  ...
  return <section className="delphi-budget" aria-label="Data budget experiment">
    <h2>How much data is enough?</h2><p className="delphi-caption">Same reference. Shared samples at every budget. Full-grid errors use a fixed evaluation domain.</p>
    <DelphiTabs label="Budget metric" value={measure} panelId="delphi-budget-plot" onChange={setMeasure} options={[{ value: "rmse", label: "RMSE" }, { value: "mae", label: "MAE" }, { value: "max_abs_error", label: "Max error" }, { value: "fit_ms", label: "Fit time" }, { value: "inference_ms", label: "Inference time" }]} />
    {points.length ? <><div className="delphi-budget-plot" id="delphi-budget-plot" role="tabpanel" aria-label="Budget metric plot"><ScientificPlot data={traces} layout={layout} label={`${measure} versus training sample count`} /></div><div className="delphi-budget-picks">{points.map(point => <button key={point.count} disabled={!point.result} title={!point.result ? "Surface evicted from memory; rerun this budget to inspect it." : "Open completed surface and metrics"} onClick={() => point.result && onSelect(point.result)}>{point.count} samples · inspect</button>)}</div>
      <details className="delphi-disclosure"><summary>Budget values · accessible table</summary><div className="delphi-table-scroll" tabIndex={0}><table><thead><tr><th>Samples</th><th>Method</th><th>{measure} ({unit})</th></tr></thead><tbody>{points.flatMap(point => point.methods.map(model => {
      ...
      }))}</tbody></table></div></details></> : <div className="delphi-empty">Choose up to four budgets, then run the experiment. Completed budgets remain available if you cancel.</div>}
  </section>;
}
